  ...
  async function fetchDashboardData(uid: string) {
    ...
    try {
      ...
      const riskyDocs = allDocs.filter(
        (d: any) => d.riskLevel === 'HIGH' || d.riskLevel === 'MEDIUM'
      );
      ...
      const recentDocuments = recentSnap.docs.map((doc) => ({ id: doc.id, ...doc.data() }));

      // ✅ Recent risky docs
      const recentRiskDocs = allDocs
        .filter((d: any) => d.riskLevel === 'HIGH' || d.riskLevel === 'MEDIUM')
        .slice(0, 3);
      ...
      setRecentRisks(
        recentRiskDocs.map((r: any) => ({
          id: r.id,
          title: r.analysis?.metadata?.type || r.fileName || 'Untitled Document',
          document: { id: r.id, name: r.fileName || 'Unnamed' },
          severity: r.riskLevel || 'LOW',
        }))
      );
    } catch (err) {
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <StatCard title="Documents Processed" value={stats.documentsProcessed} change="+12%" isLoading={isLoading} />
        <StatCard title="Documents with Risks" value={stats.riskyDocuments} change="-3%" positive={false} isLoading={isLoading} />
        <StatCard title="Avg. Processing Time" value={stats.averageProcessingTime} change="-15%" positive={true} isLoading={isLoading} />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          <div className="flex justify-between items-center mb-4">
            ...
            <Link href="/upload" className="text-sm text-blue-600 hover:underline">Upload New</Link>
          </div>

          {isLoading ? (
            <div className="animate-pulse space-y-3">
              {[1, 2, 3].map((i) => <div key={i} className="h-14 bg-gray-200 rounded-md"></div>)}
            </div>
          ) : recentDocs.length > 0 ? (
                  ...
                  <div className="flex items-center">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">
                        {doc.uploadDate ? new Date(doc.uploadDate).toLocaleDateString() : 'No date'}
                      </p>
                    </div>
                  </div>
                  <Link href={`/documents/${doc.id}`} className="text-blue-500 hover:underline">View</Link>
                </div>
              ))}
            </div>
          ) : (
            <div className="text-center p-4 text-gray-500">
              No documents found.{' '}
              <Link href="/upload" className="text-blue-600 hover:underline">Upload some</Link>?
            </div>
          )}
        </div>
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          <div className="flex justify-between items-center mb-4">
            ...
            <Link href="/reports" className="text-sm text-blue-600 hover:underline">View All Reports</Link>
          </div>
