...
export default function ReportsPage() {
  ...
  // ✅ Fetch reports with userId
  const fetchReports = async (uid: string) => {
    try {
      setIsLoading(true);
      const res = await fetch(`/api/reports?userId=${uid}`);
  ...
  // ✅ Local status update (UI only)
  const handleStatusUpdate = (id: string, newStatus: 'Open' | 'Reviewing' | 'Resolved') => {
    setLocalStatuses(prev => ({ ...prev, [id]: newStatus }));
  };

  const getStatus = (report: RiskReport) => (localStatuses[report.id] || report.status) as 'Open' | 'Reviewing' | 'Resolved';
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <Link href="/documents" className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">
          View Documents
        </Link>
      </div>
      ...
      <div className="bg-white p-6 rounded-lg shadow mb-6">
        ...
        {isLoading ? (
          ...
          <div className="text-center py-10">
            ...
            <button className="text-blue-600 hover:underline" onClick={() => user && fetchReports(user.uid)}>Retry</button>
          </div>
              ...
              return (
                <div key={report.id} className="border border-gray-200 p-4 rounded-lg hover:bg-gray-50">
                  ...
                  <div className="flex flex-wrap justify-between items-center text-sm">
                    ...
                    <div className="text-gray-500">Detected {formatDate(report.detectedAt)}</div>
                  </div>
          ...
          <div className="text-center py-10">
            ...
            <button onClick={() => { setFilter('all'); setStatusFilter('all'); setSearchTerm(''); }} className="text-blue-600 hover:underline">
              Reset filters
            </button>
          </div>
        )}
          ...
          <>
            <div className="flex flex-wrap gap-4 mb-4">
              <div className="bg-red-50 px-4 py-3 rounded-lg border border-red-100">
                ...
                <p className="text-2xl font-bold text-red-700">{reports.filter(r => r.severity === 'High').length}</p>
              </div>
              <div className="bg-yellow-50 px-4 py-3 rounded-lg border border-yellow-100">
                ...
                <p className="text-2xl font-bold text-yellow-700">{reports.filter(r => r.severity === 'Medium').length}</p>
              </div>
              <div className="bg-blue-50 px-4 py-3 rounded-lg border border-blue-100">
                ...
                <p className="text-2xl font-bold text-blue-700">{reports.filter(r => r.severity === 'Low').length}</p>
              </div>
              <div className="bg-green-50 px-4 py-3 rounded-lg border border-green-100">
                ...
                <p className="text-2xl font-bold text-green-700">
                  {Object.values(localStatuses).filter(s => s === 'Resolved').length}
                </p>
              </div>
            </div>
            <p className="text-sm text-gray-600">
              A total of {reports.length} risks identified across{' '}
              {new Set(reports.map(r => r.document.id)).size} documents.{' '}
              {reports.filter(r => getStatus(r) !== 'Resolved').length} risks remain unresolved.
            </p>
          </>
        )}
      </div>
